...
#define USE_ISPSPITXRX_SEPERATE
// If define USE_ISPSPITXRX_SEPERATE, we will call xxAddWrite + xxAddRead; otherwise we call xxAddWriteRead
...
uint8_t Read8BitWFRegister(uint8_t regId)
{
    ...
    txBuf[1] = 0xff; txBuf[2] = 0xff;  //have to be 0x00 or oxff for DRV_SPI_BufferAddWriteRead()
    WF_SpiEnableChipSelect();
#if defined(USE_ISPSPITXRX_SEPERATE)
    IspSpiTxRx_seperate(
            txBuf,
            1,
            rxBuf,
...
void MRF24W_ChipReset(void)
{
    ...
    timeoutPeriod = SYS_TMR_TickPerSecond() * 3;  /* 3000 ms */
    ...
    do
    {
        ...
        if (SYS_TMR_TickCountGet() - startTickCount >= timeoutPeriod)
        {
            SYS_ASSERT(false, "");
        }   
...
/*************************************************************************
  Function:
        void DRV_WIFI_VersionStrGet(char *p_versionStr);

  Summary:
    Gets the MRF24WG Wi-Fi Driver version number
  Description:
    Gets the MRF24WG Wi-Fi Driver version number
  Conditions:
    None.
  Input:
    p_versionStr - pointer to string buffer where version number will be written
                   by this function.
  Return:
    None
  Remarks:
    Version string will always be <= 16 characters
  *************************************************************************/
void DRV_WIFI_VersionStrGet(char *p_versionStr)
{
    strcpy(p_versionStr, DRV_WIFI_VERSION_NUMBER);
}
