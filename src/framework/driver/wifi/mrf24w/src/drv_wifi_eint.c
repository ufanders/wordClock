...
void DRV_WIFI_INT_SourceEnable(void)
{
    ...
    if (pinStatus == 0)
    {
        PLIB_INT_SourceFlagSet(INT_ID_0,MRFWB0M_INT_SOURCE);
    }

    /* enable the external interrupt */
    SYS_INT_SourceEnable(MRFWB0M_INT_SOURCE);
}
...
void DRV_WIFI_INT_Init(void)
{
    ...
    SYS_INT_SourceStatusClear(MRFWB0M_INT_SOURCE) ;                      // clear status
    SYS_INT_VectorPrioritySet(MRFWB0M_INT_VECTOR, WF_INT_PRIORITY);      //set priority level
    SYS_INT_VectorSubprioritySet(MRFWB0M_INT_VECTOR,WF_INT_SUBPRIORITY); //set sub priority level
}
