...
extern bool wf_update_begin__tcp;
...
uint8_t char_to_i(char c)
{
    ...
    {
        SYS_ASSERT(false, "this is not data\r\n");
    }
    return tmp;
}
static void parse_byte(uint8_t c)
{
    ...
    case 1:
        ...
        {    SYS_ASSERT(false, "it shold be 'x'");
        }
        break;
    case 2:
    ...
    switch(AutoUpdateTCPExampleState)
    {
       ...
       case SM_UPDATE_SENDREQUEST:
            if(!TCPIP_TCP_IsConnected(MySocket))
            {
                ...
                SYS_CONSOLE_MESSAGE("connect is lost, I am sad \r\n");
                break;
            }
            Timer = SYS_TICK_Get();
        ...
        case SM_UPDATE_READ_DATA:
            ...
            w = TCPIP_TCP_GetIsReady(MySocket);
            j = sizeof(vBuffer)-1;
            if(w!=0){ SYS_CONSOLE_PRINT("-!\r\n!-Total=%u,Current=%ud, size=%u: ",SizeOfPackage, SizeOfReceive,w);}
            while(w)
            {
                int i;
                if(w < j){j = w; }
        ...
        case SM_UPDATE_DISCONNECT:
            ...
            SYS_CONSOLE_PRINT("ImageUpdate_Size = 0x%x, ImageUpdate_Checksum=0x%x\r\n",ImageUpdate_Size,ImageUpdate_Checksum);
        ...
        case SM_UPDATE_DONE:
            ...
            #else
            if(wf_update_begin__tcp == true)
            {
                wf_update_begin__tcp = false;
                AutoUpdateTCPExampleState = SM_UPDATE_HOME;
            }
            #endif
            break;
