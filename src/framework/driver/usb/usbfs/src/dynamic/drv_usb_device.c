...
USB_ERROR DRV_USB_DEVICE_EndpointStall(DRV_HANDLE client,
                                    USB_ENDPOINT endpointAndDirection)
{
    int     direction;
    bool    interruptWasEnabled = false;
...
USB_ERROR DRV_USB_DEVICE_EndpointStallClear(DRV_HANDLE client,
                                    USB_ENDPOINT endpointAndDirection)
{
    int     direction;
    uint8_t endpoint;
        ...
        switch(lastBDTEntry->byte[0] & 0x3C)
        {
            case 0x34 :
               ...
               /* This means a setup packet has been received */
                ...
                /* currentBDTEntry at this point will point to enpoint 0
                 * BDT entry. We should get the transmit BDT entries and
                 * clear the stall conditions */

                ep0TransmitBDTEntry = currentBDTEntry + 2;

                /* ep0TranmitBDTEntry at this point should point to the
                 * EP0 transmit even BDT entry. Clearing byte 0 will clear
                 * clear the stall */
