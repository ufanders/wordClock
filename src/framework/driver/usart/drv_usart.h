...
/* USART Modes of Operation
  ...
  Remarks:
    Not all modes may be available on all microcontrollers.  Refer to the data
...
/* USART Handshake Modes
  ...
  Remarks:
    Not all modes are available on all microcontrollers.
*/
...
/* USART Initialization flags
  ...
  Remarks:
    These flags can be logically OR'd together.  They are passed into the 
...
typedef enum
{
    ...
    /* Flag to enable "wake on start" operation.  If supported and enabled,
       this feature will allow the USART to wake up the microcontroller when a
       start bit is received. This option should be selected only when the
       microcontroller device is to placed in sleep mode. Note that enabling
       this bit will also cause the first received character to be lost. Refer
       to device data sheet for more information. */
...
/* Function:
  ...
  Example: 
    ...
    numBytes = DRV_USART_Read(myUSARTHandle, readBuffer, readbufferLen) ;
    </code>
...
/* Function:
  ...
  Examples:
    <code>
    DRV_HANDLE      myUSARTHandle;    // Returned from DRV_USART_Open
    const uint8_t   writeBuffer[MY_BUFFER_SIZE];
    unsigned int    size, numBytes = 0;
    unsigned int    readbufferLen = sizeof(readBuffer);
    
    // Preinitialize writeBuffer with MY_BUFFER_SIZE bytes of data.
    
    // If the Transmit buffer is empty, write as much data as we can.
    if (DRV_USART_TRANSFER_STATUS_TRANSMIT_EMPTY & DRV_USART_TransferStatus(myUSARTHandle)
    {
        size     = min( MY_BUFFER_SIZE, DRV_USART_ReceiverBufferSizeGet(myUSARTHandle) );
        numBytes = DRV_USART_Write(myUSARTHandle, writeBuffer, size) ;
    }
    </code>
...
/* Function:
  ...
  Description:
    This routine returns identifies if the driver's receive buffer is empty or
    not. This function can be used in conjunction with the DRV_USART_Read() and
  ...
  Example:
    ...
    while( numBytes < MY_BUFFER_SIZE );
    {
        if ( !DRV_USART_ReceiverBufferIsEmpty(myUSARTHandle) )
        {
            ...
            {
                numBytes += DRV_USART_Read(myUSARTHandle,  &myBuffer[numBytes], MY_BUFFER_SIZE - numBytes ) ;
            }
            else
            {
                break;
...
/* Function:
  ...
  Example:
    ...
    {    
        ...
        {
            bytesToWrite = min( writebufferlen-numBytes, DRV_USART_TransmitBufferSizeGet(myUSARTHandle) );
            numBytes += DRV_USART_Write(myUSARTHandle, &writeBuffer[numBytes], bytesToWrite) ;
        }
    }
    </code>
...
/* Function:
  ...
  Description:
    This function returns the error(if any) associated with the last client
    request. The DRV_USART_Read() and DRV_USART_Write() will update the client
