...
TCPIP_MAC_RES           DRV_ETHMAC_PIC32StatisticsGet(TCPIP_MAC_HANDLE hMac, TCPIP_MAC_RX_STATISTICS* pRxStatistics, TCPIP_MAC_TX_STATISTICS* pTxStatistics);
...
static const TCPIP_MODULE_MAC_PIC32INT_CONFIG macPIC32INTConfigDefault = 
{
    ...
    EMAC_ETH_OPEN_FLAGS,        // ethFlags
    EMAC_PHY_CONFIG_PLAGS,      // phyFlags
    EMAC_PHY_LINK_INIT_DELAY,   // linkInitDelay
    EMAC_PHY_ADDRESS,           // phyAddress
};
...

{
    ...
    pMacD->mData.pktAllocF = (_TCPIP_PKT_ALLOC_PTR)stackData->pktAllocF;
    pMacD->mData.pktFreeF = (_TCPIP_PKT_FREE_PTR)stackData->pktFreeF;
    pMacD->mData.pktAckF = (_TCPIP_PKT_ACK_PTR)stackData->pktAckF;

    // adjust parameters
    if(initData->nTxDescriptors < ETH_PIC32_INT_MAC_MIN_TX_DESCRIPTORS)
    {
        pMacD->mData._nTxDescriptors = ETH_PIC32_INT_MAC_MIN_TX_DESCRIPTORS;
    }
    else
    {
	...
	while(1)
	{
        ...
        phyInitRes = DRV_ETHPHY_Setup(hPhyClient, initData->phyAddress, initData->ethFlags, initData->phyFlags, &linkFlags);
        ...
        {   // the _RxBuffSize is viewed as total packet size, including the ETH frame
            ...
            pRxPkt = _TCPIP_PKT_ALLOC_BY_PTR(pMacD->mData.pktAllocF, sizeof(*pRxPkt), pMacD->mData._RxBuffSize - sizeof(TCPIP_MAC_ETHERNET_HEADER), 0);
            if(pRxPkt == 0)
            {   // failed
        ...
        pMacD->mData._linkWaitTick = ((SYS_TMR_TickPerSecond() * initData->linkInitDelay) + 999 ) / 1000;
    ...
    {   // failed; release what we've allocated
        ...
        {   
            ...
            _TCPIP_PKT_FREE_BY_PTR(pMacD->mData.pktFreeF, pRxPkt);
        }
    }
...
TCPIP_MAC_RES DRV_ETHMAC_PIC32StatisticsGet(TCPIP_MAC_HANDLE hMac, TCPIP_MAC_RX_STATISTICS* pRxStatistics, TCPIP_MAC_TX_STATISTICS* pTxStatistics)
{
    ...
    if(pTxStatistics)
    {
        ...
        pMacD->mData._txStat.nTxPendBuffers += pMacD->mData._TxQueue.nNodes;
...
static void	_MACTxPacketAckCallback(void* pBuff, int buffIx, void* fParam)
{
    if(buffIx == 0)
    {
       // restore packet the buffer belongs to
        ...
        _TCPIP_PKT_ACK_BY_PTR(pMacD->mData.pktAckF, ptrPacket, TCPIP_MAC_PKT_ACK_TX_OK);
...
static TCPIP_MAC_RES _MacTxPendingPackets(DRV_ETHMAC_INSTANCE_DCPT* pMacD)
{
    ...
    while( (pPkt = (TCPIP_MAC_PACKET*)(pMacD->mData._TxQueue.head)) != 0)
    {
        ...
        {   // not transmitted
            _TCPIP_PKT_ACK_BY_PTR(pMacD->mData.pktAckF, pPkt, TCPIP_MAC_PKT_ACK_BUFFER_ERR);
        }
    }
...
static void _MacTxDiscardQueues(DRV_ETHMAC_INSTANCE_DCPT* pMacD, TCPIP_MAC_PKT_ACK_RES ackRes)
{
    ...
    {   // acknowledge the packet
        _TCPIP_PKT_ACK_BY_PTR(pMacD->mData.pktAckF, pPkt, ackRes);
    }
#endif  // defined(ETH_PIC32_INT_MAC_ISR_TX)
...
static void _MACTxAcknowledgeAckQueue(DRV_ETHMAC_INSTANCE_DCPT* pMacD)
{
    ...
    {   // acknowledge the packet; the ackRes should be set
        _TCPIP_PKT_ACK_BY_PTR(pMacD->mData.pktAckF, pPkt, TCPIP_MAC_PKT_ACK_NONE);
    }
...
static void _MacRxPacketAck(TCPIP_MAC_PACKET* pRxPkt,  const void* param)
{
    ...
    for(pSeg = pRxPkt->pDSeg; pSeg != 0; pSeg = pSeg->next)
    {
        ...
        {   // we're dead and gone
            ...
            _TCPIP_PKT_FREE_BY_PTR(pMacD->mData.pktFreeF, pCurrPkt);
        }
        else
...
static TCPIP_MAC_RES DRV_ETHMAC_PIC32MACEventInit(TCPIP_MAC_HANDLE hMac, TCPIP_MAC_EventF eventF, const void* eventParam, int intPri, int intSubPri)
{
    ...
    SYS_INT_VectorPrioritySet(ETHMAC_INTERRUPT_VECTOR, (INT_PRIORITY_LEVEL1-1)+intPri);
    SYS_INT_VectorSubprioritySet(ETHMAC_INTERRUPT_VECTOR, INT_SUBPRIORITY_LEVEL0+intSubPri);
...
void DRV_ETHMAC_Tasks_ISR( SYS_MODULE_OBJ macIndex )
{
    ...
    {   // locally acknowledge TX done activity
        ...
        while((pPkt = (TCPIP_MAC_PACKET*)TCPIP_Helper_SingleListHeadRemove(&pMacD->mData._TxQueue)) != 0)
        {
            ETH_RESULT_CODE eRes = DRV_ETHMAC_LibTxSendPacket(pMacD, (const DRV_ETHMAC_PKT_DCPT*)pPkt->pDSeg);
