...
// *****************************************************************************
/* Macro:
    _DRV_ADC_ConversionClockSelect( index, convClock )

  Summary:
    Selects the conversion clock for the ADC module defined by the index
    parameter and maps to the respective peripheral library API.

  Description:
    This macro selects the conversion clock for the ADC module defined by the index
    parameter and maps to the respective peripheral library API.

    This macro expands to the peripheral library API only if the feature is supported 
...
#if defined (_ADC_EXISTS_CONVERSION_CLOCK_SELECT)

    #define _DRV_ADC_ConversionClockSelect( index, convClock ) \
            PLIB_ADC_ConversionClockSelect( _DRV_ADC_PERIPHERAL_ID_GET ( index ), convClock )
...
// *****************************************************************************
/* Macro:
    _DRV_ADC_ConversionClockSource( index, clockSource )

  Summary:
    Selects the conversion clock source for the ADC module defined by the
    index parameter and maps to the respective peripheral library API.

  Description:
    This macro selects the conversion clock source for the ADC module defined by
    the index parameter and maps to the respective peripheral library API.
...
#if defined(_ADC_EXISTS_CONVERSION_CLOCK_SOURCE)

    #define _DRV_ADC_ConversionClockSource( index, clockSource ) \
            PLIB_ADC_ConversionClockSource( _DRV_ADC_PERIPHERAL_ID_GET ( index ), clockSource )
...
// *****************************************************************************
/* Macro:
    _DRV_ADC_ConversionTriggerSourceSelect( index, convTriggerSource )

  Summary:
    Selects the conversion trigger source for the ADC module defined by the
    index parameter and maps to the respective peripheral library API.

  Description:
    This macro selects the conversion trigger source for the ADC module defined by
    the index parameter and maps to the respective peripheral library API.
...
#if defined(_ADC_EXISTS_CONVERSION_TRIGGER_SOURCE)

    #define _DRV_ADC_ConversionTriggerSourceSelect( index, convTriggerSource ) \
            PLIB_ADC_ConversionTriggerSourceSelect( _DRV_ADC_PERIPHERAL_ID_GET ( index ), convTriggerSource )
...
// *****************************************************************************
/* Macro:
    _DRV_ADC_ResultFormatSelect( index, outputFormat )

  Summary:
    Selects the output data format for the ADC module defined by the index 
    parameter and maps to the respective peripheral library API.

  Description:
    This macro selects the output data format for the ADC module defined by
    the index parameter and maps to the respective peripheral library API.
...
#if defined(_ADC_EXISTS_OUTPUT_FORMAT_SELECT)

    #define _DRV_ADC_ResultFormatSelect( index, outputFormat ) \
            PLIB_ADC_ResultFormatSelect( _DRV_ADC_PERIPHERAL_ID_GET ( index ), outputFormat )
...
// *****************************************************************************
/* Macro:
    _DRV_ADC_VoltageReferenceSelect( index, vReference )

  Summary:
    Selects the voltage reference for the ADC module defined by the index parameter 
    and maps to the respective peripheral library API.

  Description:
    This macro selects the voltage reference for the ADC module defined by the index 
    parameter and maps to the respective peripheral library API.
...
#if defined(_ADC_EXISTS_VREF_SELECT)

    #define _DRV_ADC_VoltageReferenceSelect( index, vReference ) \
            PLIB_ADC_VoltageReferenceSelect( _DRV_ADC_PERIPHERAL_ID_GET ( index ), vReference )
...
// *****************************************************************************
/* Macro:
    _DRV_ADC_PowerState(index , adcInit)

  Summary:
    Stops the ADC module in Idle mode and maps to the respective peripheral
    library API.

  Description:
    This macro stops the ADC module in Idle mode.

    This macro expands to the peripheral library API only if the feature is 
    supported by the respective device

    The API maps to the peripheral library API PLIB_ADC_StopInIdleEnable if 
     the configuration option DRV_ADC_POWER_STATE is SYS_MODULE_POWER_IDLE_STOP.

    The API maps to the peripheral library API PLIB_ADC_StopInIdleDisable if the 
    configuration option DRV_ADC_POWER_STATE is either SYS_MODULE_POWER_IDLE_RUN 
    or SYS_MODULE_POWER_RUN_FULL.

    If none of the configuration options are mentioned and the feature exists in
    the device, it enables the feature based on the SYS module power state.

  Remarks:
    None.
*/

#if defined (_ADC_EXISTS_STOP_IN_IDLE_ENABLE)

    #if defined(DRV_ADC_POWER_STATE) && ( DRV_ADC_POWER_STATE == SYS_MODULE_POWER_IDLE_STOP )

        #define _DRV_ADC_PowerState(index , adcInit)   PLIB_ADC_StopInIdleEnable( _DRV_ADC_PERIPHERAL_ID_GET(index) )

    #elif  defined(DRV_ADC_POWER_STATE) && ( (DRV_ADC_POWER_STATE == SYS_MODULE_POWER_IDLE_RUN) || \
                                                          DRV_ADC_POWER_STATE == SYS_MODULE_POWER_RUN_FULL)

        #define _DRV_ADC_PowerState(index , adcInit)   PLIB_ADC_StopInIdleDisable( _DRV_ADC_PERIPHERAL_ID_GET(index) )

    #else

        #define _DRV_ADC_PowerState(index , adcInit )  \
                    ( ( _DRV_ADC_POWER_STATE_GET(adcInit->moduleInit.sys.powerState) == SYS_MODULE_POWER_IDLE_STOP ) ? \
                        PLIB_ADC_StopInIdleEnable( _DRV_ADC_PERIPHERAL_ID_GET( index ) ) :        \
                        0 )
    #endif
...
// *****************************************************************************
/* Macro:
    _DRV_ADC_SampleAcqusitionTimeSelect( index, acqTime )

  Summary:
    Selects the acquisition time for the ADC module defined by the index parameter 
    and maps to the respective peripheral library API.

  Description:
    This macro selects the acquisition time for the ADC module defined by the 
    index parameter and maps to the respective peripheral library API.
...
#if defined(_ADC_EXISTS_ACQUISITION_TIME)

    #define _DRV_ADC_SampleAcqusitionTimeSelect( index, acqTime ) \
            PLIB_ADC_SampleAcqusitionTimeSelect( _DRV_ADC_PERIPHERAL_ID_GET ( index ), acqTime )
...
// *****************************************************************************
/* Macro:
    _DRV_ADC_SampleAutoStartEnable(index)

  Summary:
    Enables auto-sampling mode for the ADC module defined by the index parameter 
    and maps to the respective peripheral library API.

  Description:
    This macro enables auto-sampling mode for the ADC module defined by the index
    parameter and maps to the respective peripheral library API
...
*/
    ...
    #define _DRV_ADC_SampleAutoStartEnable( index )     \
            PLIB_ADC_SampleAutoStartEnable( _DRV_ADC_PERIPHERAL_ID_GET ( index ) )
...
// *****************************************************************************
/* Macro:
    _DRV_ADC_ConversionStopSequenceEnable(index)

  Summary:
    Stops a conversion when the first ADC interrupt is generated for the ADC 
    module defined by the index parameter and maps to the respective peripheral 
    library API.

  Description:
    This macro stops conversions when the first ADC interrupt is generated for
    the ADC module defined by the index parameter and maps to the respective 
    peripheral library API.
...
*/
    ...
    #define _DRV_ADC_ConversionStopSequenceEnable( index )    \
            PLIB_ADC_ConversionStopSequenceEnable( _DRV_ADC_PERIPHERAL_ID_GET ( index ) )
...
// *****************************************************************************
/* Macro:
    _DRV_ADC_ModeAlternateInputSampleEnable(index)

  Summary:
    Enable the alternate input sampling feature of the ADC.

  Description:
    This macro enables the alternate input sampling feature of the ADC module
    defined by the index parameter and maps to the respective peripheral library 
    API.
...
#if defined(_ADC_EXISTS_ALTERNATE_SAMPLING_SELECT) && (DRV_ADC_CONFIG_ALTERNATE_INPUT_SAMPLING_ENABLE == true)

    #define _DRV_ADC_ModeAlternateInputSampleEnable( index )    \
            PLIB_ADC_ModeAlternateInputSampleEnable( _DRV_ADC_PERIPHERAL_ID_GET ( index ) )
...
// *****************************************************************************
/* Macro:
    _DRV_ADC_SamplesPerInterruptSelect( index, samplesPerInterrupt )

  Summary:
    Selects the number of samples per interrupt for the ADC module defined
    by the index parameter and maps to the respective peripheral library API.

  Description:
    This macro selects the number of samples per interrupt for the ADC module 
    defined by the index parameter and maps to the respective peripheral library 
    API.
...
#if defined(_ADC_EXISTS_SAMPLES_PER_INTERRUPT_SELECT)

    #define _DRV_ADC_SamplesPerInterruptSelect( index, samplesPerInterrupt ) \
            PLIB_ADC_SamplesPerInterruptSelect( _DRV_ADC_PERIPHERAL_ID_GET ( index ), samplesPerInterrupt )
...
// *****************************************************************************
/* Macro:
    _DRV_ADC_MuxAInputScanEnable(index)

  Summary:
    Enables a MUXA input scan for the ADC module defined by the index
    parameter and maps to the respective peripheral library API.

  Description:
    This macro enables MUXA input scan for the ADC module defined by the index
    parameter and maps to the respective peripheral library API,
...
*/
    ...
    #define _DRV_ADC_MuxAInputScanEnable( index )       \
            PLIB_ADC_MuxAInputScanEnable( _DRV_ADC_PERIPHERAL_ID_GET ( index ) );
#else
...
#if defined(_ADC_EXISTS_CONVERSION_RESULT_IN_BUFFER_ARRAY)

    #define _DRV_ADC_ReadSample( index, bufferIndex )   \
            _DRV_ADC_ReadSampleFromArray( index, bufferIndex )

#elif defined(_ADC_EXISTS_CONVERSION_RESULT_IN_SINGLE_BUFFER)

    #define _DRV_ADC_ReadSample( index, bufferIndex )   \
            PLIB_ADC_ResultGet( index )
