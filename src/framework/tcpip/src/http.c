    ...
    {   // first time we're run
        ...
        if(httpConnNo)
        {
            ...
            for(conn = 0; conn < httpConnNo ; conn++)
            {
                ...
                TCPIP_TCPSSL_ListenerAdd(pHttpCon->socket, TCPIP_HTTP_SERVER_SECURE_PORT);
#endif
        ...
        switch(pHttpCon->sm)
        {
            ...
            case SM_HTTP_IDLE:
                ...
                {// Clear out state info and move to next state
                    ...
                    pHttpCon->httpTick = SYS_TMR_TickCountGet() + HTTP_TIMEOUT*SYS_TMR_TickPerSecond();
            ...
            case SM_HTTP_PARSE_REQUEST:
                ...
                {// First line isn't here yet
                    ...
                    if(SYS_TMR_TickCountGet() > pHttpCon->httpTick)
                ...
                if(pHttpCon->data[lenB-1] != '/') {
                    ...
                    {
                        SYS_ERROR(SYS_ERROR_WARN, " HTTP: URL exceeds allocated space!");
                    }
                ...
                if(pHttpCon->file == SYS_FS_HANDLE_INVALID)
                {
                    ...
                    {
                        strncpy((char*)pHttpCon->data + lenB, HTTPS_DEFAULT_FILE, httpConnDataSize - lenB);
                        lenB += strlen(HTTPS_DEFAULT_FILE);
                    }
                    else
#endif
                    {
                        strncpy((char*)pHttpCon->data + lenB, HTTP_DEFAULT_FILE, httpConnDataSize - lenB);
...
#if defined(HTTP_FILE_UPLOAD) && defined(NVM_DRIVER_V080_WORKAROUND)
...
#endif  // defined(HTTP_FILE_UPLOAD) && defined(NVM_DRIVER_V080_WORKAROUND)
...
/*****************************************************************************
  ...
  Return Values:
    true  - no more data can be processed for this call and the big TCPIP_HTTP_Process loop has to be broken
    ...
    false - TCPIP_HTTP_Process can continue, no break needed

  Note:
    the function sets the pHttpCon->TxFile.fileTxDone flag which also breaks the TCPIP_HTTP_Process loop
    ...
    switch(pHttpCon->file_sm)
    {
        ...
        SYS_FS_FileSeek(DynVarFilePtr, pHttpCon->TxFile.DynRcrdRdCount, SYS_FS_SEEK_SET);               
            ...
            {                
                ...
                if(availbleTcpBuffSize >= sizeof(sendDataBuffer))
                {
                    if((pHttpCon->TxFile.dynVarRcrdOffset-pHttpCon->TxFile.bytesReadCount) >= sizeof(sendDataBuffer))
                    {
                        len = SYS_FS_FileRead(pHttpCon->file, sendDataBuffer, sizeof(sendDataBuffer));
                        _HTTP_FileRdCheck(len==sizeof(sendDataBuffer), __FILE__, __LINE__);
                        TCPIP_TCP_ArrayPut(pHttpCon->socket, sendDataBuffer, sizeof(sendDataBuffer));
                        pHttpCon->TxFile.numBytes -=sizeof(sendDataBuffer);
                        pHttpCon->TxFile.bytesReadCount+=sizeof(sendDataBuffer);
                    }
                    else
                    {
                ...
                else if(availbleTcpBuffSize != 0)
                {
                    if(pHttpCon->TxFile.bytesReadCount != pHttpCon->TxFile.dynVarRcrdOffset )
                    {
                        ...
                        {
                            len = SYS_FS_FileRead(pHttpCon->file, sendDataBuffer, availbleTcpBuffSize);
        ...
        case SM_SERVE_TEXT_DATA:
            ...
            if(availbleTcpBuffSize >= sizeof(sendDataBuffer))
            {
                ...
                {
                    len = SYS_FS_FileRead(pHttpCon->file, sendDataBuffer, pHttpCon->TxFile.numBytes);
...
static HTTP_IO_RESULT TCPIP_HTTP_MPFSUpload(HTTP_CONN* pHttpCon)
{
    ...
    switch(pHttpCon->httpStatus)
    {
        ...
        case HTTP_MPFS_UP:
            ...
            {// Found it, so remove all data up to and including
                ...
                {   // Read as Ver 2.1
                    ...
                    {
                        pHttpCon->uploadBufferStart = (uint8_t*)SystemMalloc(DRV_NVM_PAGE_SIZE);
        ...
        case HTTP_MPFS_ERROR:
            if(pHttpCon->uploadBufferStart != 0)
            {
                SystemFree(pHttpCon->uploadBufferStart);
        ...
        case HTTP_MPFS_OK:
            ...
            while(SYS_FS_MEDIA_MANAGER_BufferStatusGet(MPFS_UPLOAD_DISK_NO, bufferHandle) != SYS_FS_MEDIA_BUFFER_COMPLETED);
...
void TCPIP_HTTP_FileInclude(HTTP_CONN_HANDLE connHandle, const uint8_t* cFile)
{
    ...
    if(availbleTcpBuffSize >= sizeof(incDataBuffer))
    {
        if(pHttpCon->TxFile.numBytesHdrFile <= sizeof(incDataBuffer))
        {
            len = SYS_FS_FileRead(fp, incDataBuffer, pHttpCon->TxFile.numBytesHdrFile);
        ...
        {
            len = SYS_FS_FileRead(fp, incDataBuffer, sizeof(incDataBuffer));
            _HTTP_FileRdCheck(len==sizeof(incDataBuffer), __FILE__, __LINE__);
            TCPIP_TCP_ArrayPut(pHttpCon->socket, incDataBuffer, sizeof(incDataBuffer));
            pHttpCon->TxFile.numBytesHdrFile -=sizeof(incDataBuffer);
        }
    }
    else //if(availbleTcpBuffSize != 0)
    {
        ...
        {
            len = SYS_FS_FileRead(fp, incDataBuffer, availbleTcpBuffSize);
            _HTTP_FileRdCheck(len==availbleTcpBuffSize, __FILE__, __LINE__);
