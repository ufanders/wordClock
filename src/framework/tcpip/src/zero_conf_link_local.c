            ...
            switch ( zgzc_wait_for(&hZcll->random_delay, &hZcll->event_time, &hZcll->time_recorded) )
            {
                case ZGZC_STARTED_WAITING:
                    ...
                    if (hZcll->probe_count == 0)
                    {
                        ...
                        hZcll->random_delay = (_zcll_rand() % (PROBE_WAIT * SYS_TMR_TickPerSecond()));
                    ...
                    else if (hZcll->probe_count < PROBE_NUM)
                    {
                        ...
                        hZcll->random_delay = ( (_zcll_rand() % ((PROBE_MAX-PROBE_MIN) * SYS_TMR_TickPerSecond()) ) +
                                (PROBE_MIN * SYS_TMR_TickPerSecond()) );
                    ...
                    else
                    {
                        ...
                        hZcll->random_delay = (ANNOUNCE_WAIT * SYS_TMR_TickPerSecond());
                        DEBUG0_ZCLL_MESG(zeroconf_dbg_msg,"ANNOUNCE_WAIT delay [%d]: %ld ticks\r\n",
                                hZcll->probe_count,
                                hZcll->random_delay /*SYS_TMR_TickPerSecond() */);
                    }
                        ...
                        ,&hZcll->time_recorded) )
            {
                case ZGZC_STARTED_WAITING:
                    ...
                    else
                    {
                        ...
                        hZcll->random_delay = (ANNOUNCE_INTERVAL * SYS_TMR_TickPerSecond());
                    }
                    // Intentional fall-through
            ...
            if (hZcll->zcll_flags.defended)
            {
                switch ( zgzc_wait_for(&hZcll->random_delay, &hZcll->event_time, &hZcll->time_recorded) )
                {
                    case ZGZC_STARTED_WAITING:
                        ...
                        hZcll->random_delay = (DEFEND_INTERVAL * SYS_TMR_TickPerSecond());
