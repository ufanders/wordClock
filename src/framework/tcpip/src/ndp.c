                    ...
                    const void* ndpData)
{
    ...
    gInitialDelay = SYS_TMR_TickCountGet() + (((LFSRRand() % 40) * SYS_TMR_TickPerSecond() * MAX_RTR_SOLICITATION_DELAY) / 40);
...
/*****************************************************************************
  Function:
    void TCPIP_NDP_TimeoutHandler (uint32_t curSysTick)
...
IPV6_HEAP_NDP_NC_ENTRY * TCPIP_NDP_NborEntryCreate (TCPIP_NET_IF * pNetIf, IPV6_ADDR * remoteIPAddr, TCPIP_MAC_ADDR * remoteMACAddr, unsigned char initialState, unsigned char routerFlag, IPV6_ADDR_STRUCT * preferredSource)
{
    ...
    if (neighborPointer != NULL)
    {
        ...
        TCPIP_Helper_ProtectedSingleListInitialize (&neighborPointer->queuedPackets);
        if (routerFlag)
...
IPV6_HEAP_NDP_DC_ENTRY * TCPIP_NDP_DestCacheEntryCreate (TCPIP_NET_IF * pNetIf, IPV6_ADDR * remoteIPAddress, unsigned long linkMTU, IPV6_HEAP_NDP_NC_ENTRY * neighbor)
{
    ...
    if (destinationPointer != NULL)
    {
        ...
        if (linkMTU != IPV6_DEFAULT_LINK_MTU)
            destinationPointer->pathMTUIncreaseTimer = SYS_TMR_TickCountGet() + (SYS_TMR_TickPerSecond() * IPV6_MTU_INCREASE_TIMEOUT);
        else
...
IPV6_HEAP_NDP_NC_ENTRY * TCPIP_NDP_NborEntryDelete (TCPIP_NET_IF * pNetIf, IPV6_HEAP_NDP_NC_ENTRY * entry)
{
    ...
    while (entry->queuedPackets.list.nNodes != 0)
    {
        pkt = (IPV6_PACKET *)TCPIP_Helper_ProtectedSingleListHeadRemove(&entry->queuedPackets);;
        if (pkt->ackFnc)
        {
                ...
                case DAD_STATE_TRANSMIT:
                    {
                        ...
                        if (pkt != NULL)
                        {
                            ...
                            gDuplicateAddrDetectState[i].timer = SYS_TMR_TickCountGet() + ((SYS_TMR_TickPerSecond() * TCPIP_IPV6_InterfaceConfigGet(gDuplicateAddrDetectState[i].netConfig)->retransmitTime) / 1000);
    ...
    for(netIx=0; netIx< nStackIfs; netIx++)
    {
        ...
        switch (pRs->state)
        {
            ...
            case RS_STATE_TRANSMIT:
                ...
                pRs->transmitAttempt++;
                pRs->timer = SYS_TMR_TickCountGet() + (SYS_TMR_TickPerSecond() * RTR_SOLICITATION_INTERVAL);
...
void TCPIP_NDP_ReachabilitySet (TCPIP_NET_IF * pNetIf, IPV6_HEAP_NDP_NC_ENTRY * neighborPointer, NEIGHBOR_UNREACHABILITY_DETECT_STATE newState)
{
    ...
    if (newState == NDP_STATE_DELAY)
    {
        ndpNUDCount++;
        neighborPointer->unansweredProbes = 0;
        neighborPointer->nextNUDTime = SYS_TMR_TickCountGet() + (SYS_TMR_TickPerSecond() * DELAY_FIRST_PROBE_TIME);

    }
    else if (newState == NDP_STATE_REACHABLE)
    {
        neighborPointer->nextNUDTime = SYS_TMR_TickCountGet() + (TCPIP_IPV6_InterfaceConfigGet(pNetIf)->reachableTime * SYS_TMR_TickPerSecond());
        neighborPointer->flags.bResolvingAddress = false;
    }
    else if (newState == NDP_STATE_STALE)
    {
        neighborPointer->staleStateTimeout = SYS_TMR_TickCountGet() + (SYS_TMR_TickPerSecond() * IPV6_NEIGHBOR_CACHE_ENTRY_STALE_TIMEOUT);
    }
}
...
void TCPIP_NDP_NborCacheLinkLayerAddressUpdate (TCPIP_NET_IF * pNetIf, IPV6_HEAP_NDP_NC_ENTRY * neighborPointer, TCPIP_MAC_ADDR * linkLayerAddr, uint8_t reachability)
{
    ...
    switch (neighborPointer->reachabilityState)
    {
        ...
        case NDP_STATE_PROBE:
            while (neighborPointer->queuedPackets.list.nNodes != 0)
            {
                memcpy (&((IPV6_PACKET *)(neighborPointer->queuedPackets.list.head))->remoteMACAddr, &neighborPointer->remoteMACAddr, sizeof(TCPIP_MAC_ADDR));
                if (TCPIP_IPV6_PacketTransmit ((IPV6_PACKET *)neighborPointer->queuedPackets.list.head))
                {
                    IPV6_PACKET * pkt;
                    pkt = (IPV6_PACKET *)TCPIP_Helper_ProtectedSingleListHeadRemove(&neighborPointer->queuedPackets);;
                    if (pkt->ackFnc)
                    {
                        (*pkt->ackFnc)(pkt, false, pkt->ackParam);
                    }
                        ...
                        if (neighborPointer->unansweredProbes < MAX_UNICAST_SOLICIT)
                        {
                            ...
                            else
                            {
                                if (TCPIP_IPV6_TxIsPutReady(pkt, sizeof(NDP_OPTION_LLA)))
                                {
                                    ...
                                    neighborPointer->nextNUDTime = SYS_TMR_TickCountGet() + ((TCPIP_IPV6_InterfaceConfigGet(pNetIf)->retransmitTime * SYS_TMR_TickPerSecond()) / 1000);
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if (TCPIP_IPV6_TxIsPutReady(pkt, sizeof(NDP_OPTION_LLA)))
                                {
                                    ...
                                    neighborPointer->unansweredProbes++;
                                    neighborPointer->nextNUDTime = SYS_TMR_TickCountGet() + ((pIpv6Config->retransmitTime * SYS_TMR_TickPerSecond()) / 1000);
