             ...
             const FTP_MODULE_CONFIG* ftpData)
{
    ...
    if(sTCPIPFTPServerCount == 0)
    {
        if(ftpData)
        {
            if((strlen((char*)ftpData->userName)> TCPIP_FTP_USER_NAME_LEN) ||
                        (strlen((char*)ftpData->password)> TCPIP_FTP_PASSWD_LEN))
               return false;
            ftpConfigDefault.dataSktRxBuffSize = ftpData->dataSktRxBuffSize;
            ftpConfigDefault.dataSktTxBuffSize = ftpData->dataSktTxBuffSize;
            strncpy((char*)ftpConfigDefault.userName,(char*)ftpData->userName,TCPIP_FTP_USER_NAME_LEN);
            strncpy((char*)ftpConfigDefault.password,(char*)ftpData->password,TCPIP_FTP_PASSWD_LEN);
        }
        else
        {
...
bool TCPIP_FTP_Server(TCPIP_NET_IF * pNetIf)
{
    ...
    else if ( pFTPDcpt->ftpSm != TCPIP_FTP_SM_NOT_CONNECTED )
    {
       ...
       if ( currentTick >= (TCPIP_FTP_TIMEOUT * SYS_TMR_TickPerSecond()) )
       {
           pFTPDcpt->ftpSysTicklastActivity = SYS_TMR_TickCountGet();
    ...
    switch(pFTPDcpt->ftpCommandSm)
    {
       ...
       case TCPIP_FTP_CMD_SM_SEND_DIR:
        ...
        case TCPIP_FTP_CMD_SM_SEND_DIR_HEADER:
            ...
            fp = pFTPDcpt->fileDescr;
            TCPIP_Helper_ProtectedSingleListInitialize(&DirectoryFileList);
            while(1)
            {
                memset(fs_stat.fname,0,sizeof(fs_stat.fname));
                memset(fs_stat.lfname,0,sizeof(fs_stat.lfname));
...
static bool TCPIP_FTP_LSCmd(TCPIP_FTP_DCPT* pFTPDcpt)
{
    ...
    switch(pFTPDcpt->ftpCommandSm)
    {
        ...
        case TCPIP_FTP_CMD_SM_SEND_FILE:
            ...
            // for Windows ftp client , NLST command considers -a as a file argumnet and
