...
static bool     _TcpClientSocketConnect(TCB_STUB* pSkt)
{
    ...
    pSkt->retryCount = 0;
    pSkt->retryInterval = (SYS_TMR_TickPerSecond()/4);
...
uint16_t TCPIP_TCP_ArrayPut(TCP_SOCKET hTCP, const uint8_t* data, uint16_t len)
{
	...
	else if(!pSkt->Flags.bTimer2Enabled)
	{
		...
		pSkt->eventTime2 = SYS_TMR_TickCountGet() + (TCP_AUTO_TRANSMIT_TIMEOUT_VAL * SYS_TMR_TickPerSecond())/1000;
	}
...
uint16_t TCPIP_TCP_ArrayGet(TCP_SOCKET hTCP, uint8_t* buffer, uint16_t len)
{
    ...
    {   // not enough data freed to generate a window update
        ...
        {
            pSkt->Flags.bTimer2Enabled = true;
            pSkt->eventTime2 = SYS_TMR_TickCountGet() + (TCP_WINDOW_UPDATE_TIMEOUT_VAL * SYS_TMR_TickPerSecond())/1000;
        }
    }
	...
	for(hTCP = 0; hTCP < TcpSockets; hTCP++)
	{
        ...
        {   // existing socket
            ...
            if(pSkt->Flags.bTimer2Enabled)
            {
                ...
                if(SYS_TMR_TickCountGet() >= pSkt->eventTime2)
                {
                    vFlags = ACK;
                }
            }
            ...
            if(pSkt->Flags.bDelayedACKTimerEnabled)
            {
                ...
                if(SYS_TMR_TickCountGet() >= pSkt->delayedACKTime)
                {
                    vFlags = ACK;
                }
            }
            ...
            if(pSkt->smState == TCP_CLOSE_WAIT)
            {
                ...
                if(SYS_TMR_TickCountGet() >= pSkt->closeWaitTime)
                {
                    vFlags = FIN | ACK;
            ...
            if(!pSkt->Flags.bTimerEnabled)
            {
                ...
                if(pSkt->smState == TCP_ESTABLISHED)
                {
                    ...
                    if(SYS_TMR_TickCountGet() < pSkt->eventTime)
                    {
                        continue;
                    }
                    ...
                    pSkt->eventTime = SYS_TMR_TickCountGet() + (TCP_KEEP_ALIVE_TIMEOUT * SYS_TMR_TickPerSecond())/1000;
                }
#endif
                continue;
            }
            ...
            switch(pSkt->smState)
            {
                case TCP_SYN_SENT:
                    ...
                    if(pSkt->retryCount >= (TCP_MAX_RETRIES - 1))
                    {
                        ...
                        pSkt->retryInterval = ((TCP_START_TIMEOUT_VAL * SYS_TMR_TickPerSecond())/1000) << (TCP_MAX_RETRIES-1);
                    }
                    break;
...
static _TCP_SEND_RES _TcpSend(TCB_STUB* pSkt, uint8_t vTCPFlags, uint8_t vSendFlags)
{
    ...
    if(len || (vTCPFlags & (SYN | FIN)))
    {
        ...
        {
            pSkt->retryCount = 0;
            pSkt->retryInterval = (TCP_START_TIMEOUT_VAL * SYS_TMR_TickPerSecond())/1000;
        }	
        ...
        case TCP_CLOSING:
            ...
            if(pSkt->smState == TCP_FIN_WAIT_1)
            {
                ...
                if((pSkt->MySEQ == localAckNumber) && pSkt->flags.bFINSent)
                {
                    ...
                    pSkt->closeWaitTime = SYS_TMR_TickCountGet() + (TCP_FIN_WAIT_2_TIMEOUT * SYS_TMR_TickPerSecond())/1000;
                    pSkt->Flags.bTimerEnabled = 1;
    ...
    if(wSegmentLength)
    {
        ...
        else
        {
            ...
            if(!pSkt->Flags.bDelayedACKTimerEnabled)
            {
                ...
                pSkt->delayedACKTime = SYS_TMR_TickCountGet() + (TCP_DELAYED_ACK_TIMEOUT * SYS_TMR_TickPerSecond())/1000;
        ...
        if(pSkt->RemoteSEQ + 1 == localSeqNumber + (uint32_t)wSegmentLength)
        {
            ...
            switch(pSkt->smState)
            {
                ...
                case TCP_ESTABLISHED:
                    ...
                    pSkt->closeWaitTime = SYS_TMR_TickCountGet() + (TCP_CLOSE_WAIT_TIMEOUT * SYS_TMR_TickPerSecond())/1000;
