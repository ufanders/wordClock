...
uint32_t TCPIP_SNMPv3_AuthEngineTimeTickGet(void)
{
    ...
    timeStamp = (SYS_TMR_TickCountGet() * 100ull)/SYS_TMR_TickPerSecond();
...
bool TCPIP_SNMPV3_EngineUserDataBaseSet(TCPIP_SNMPV3_USERDATABASECONFIG_TYPE userDataBaseType,uint8_t len,uint8_t userIndex,void *val)
{
    ...
    switch(userDataBaseType)
    {
        case SNMPV3_USERNAME_CONFIG_TYPE:
            ...
            memset(snmpv3EngnDcptMemoryStubPtr->UserInfoDataBase[userIndex].userName,'\0',USER_SECURITY_NAME_LEN);
        ...
        case SNMPV3_HASHTYPE_CONFIG_TYPE:
            ...
            {
                snmpv3EngnDcptMemoryStubPtr->UserInfoDataBase[userIndex].userHashType = SNMPV3_HAMC_MD5;
            }
        ...
        case SNMPV3_TARGET_SECURITY_NAME_TYPE:
        {
            ...
            for(index=0;index<SNMPV3_USM_MAX_USER;index++)
            {
...
//            memset(snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[userIndex].userSecurityName,'\0',USER_SECURITY_NAME_LEN);
//            strncpy((char*)snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[userIndex].userSecurityName,(char*)val,len);
            return true;
        }
...
void TCPIP_SNMPv3_DynAllocMemFree(void)
{
    ...
    if(Snmpv3StackDcptStubPtr->InPduWholeMsgBuf.wholeMsgHead != NULL)
    {
        ...
        Snmpv3StackDcptStubPtr->InPduWholeMsgBuf.wholeMsgLen = 0;
...
bool TCPIP_SNMPv3_V3MsgDataProcess(PDU_INFO* pduDbPtr)
{
    ...
    /* pass the data to wire*/
    {
        ...
        Snmpv3StackDcptStubPtr->OUTPduWholeMsgBuf.wholeMsgHead=
                    (uint8_t*)(TCPIP_HEAP_Calloc(snmpPktProcsMemPtrsInfo.snmpHeapMemHandler,1,(size_t)Snmpv3StackDcptStubPtr->OUTPduWholeMsgBuf.wholeMsgLen+4+16));
        if(Snmpv3StackDcptStubPtr->OUTPduWholeMsgBuf.wholeMsgHead == NULL)
            return false;
        ...
        *outBufPtr++=tempScopedData.head[i++];//0x30
        ...
        for(i=0;i < Snmpv3StackDcptStubPtr->OUTPduWholeMsgBuf.wholeMsgLen;i++)
        {
...
static bool _SNMPv3_CheckIfv3ValidOID(uint8_t* oid, uint8_t* len)
{
   ...
   tempLen = TCPIP_SNMPv3_ProcessBuffDataGet(Snmpv3StackDcptStubPtr->ScopedPduRequstBuf,++Snmpv3StackDcptStubPtr->ScopedPduDataPos);
    ...
    if (tempLen > (uint8_t)SNMP_OID_MAX_LEN )
        return false;
...
bool TCPIP_SNMPv3_Notify(SNMP_ID var, SNMP_VAL val, SNMP_INDEX index,uint8_t targetIndex)
{
    ...
    {
        Snmpv3TrapFileDescrptr = SYS_FS_FileOpen_Wrapper((const char*)SNMP_BIB_FILE_NAME,SYS_FS_FILE_OPEN_READ);
        if(Snmpv3TrapFileDescrptr == SYS_FS_HANDLE_INVALID)
        {
            TCPIP_UDP_Close(snmpStkDcptMemStubPtr->SNMPNotifyInfo.socket);
            snmpStkDcptMemStubPtr->SNMPNotifyInfo.socket =  INVALID_UDP_SOCKET;
    ...
    Snmpv3StackDcptStubPtr->TrapOUTPduWholeMsgBuf.wholeMsgHead=
            (uint8_t*)(TCPIP_HEAP_Calloc(snmpPktProcsMemPtrsInfo.snmpHeapMemHandler,1,(size_t)Snmpv3StackDcptStubPtr->TrapOUTPduWholeMsgBuf.wholeMsgLen+4+16));
    ...
    *outBufPtr++=Snmpv3StackDcptStubPtr->TrapScopdPduRespnsBuf.head[i++];//0x30
    ...
    for(;i<Snmpv3StackDcptStubPtr->TrapOUTPduWholeMsgBuf.wholeMsgLen;i++)
    {
