...
static void ReportBW_Jitter_Loss(tIperfReport reportType)
{
    ...
    switch ( reportType )
    {
        case INTERVAL_REPORT:
            ...
            sec = (currentTime- gIperfState.lastCheckTime)/SYS_TMR_TickPerSecond();
			msec = ((double) (currentTime - gIperfState.lastCheckTime)) / (((double)(SYS_TMR_TickPerSecond()))/1000);
            ...
            sec = (gIperfState.lastCheckTime - gIperfState.startTime)/SYS_TMR_TickPerSecond();

            (gIperfState.pCmdIO->pCmdApi->print)(cmdIoParam, "    - [%2lu- %2lu sec] %3lu/ %3lu (%2lu%%)    %4lu Kbps\r\n",
                      (unsigned long)sec, 
                      (unsigned long)sec + ( (unsigned long) (gIperfState.mInterval/SYS_TMR_TickPerSecond()) ),
        ...
        case SESSION_REPORT:
           ...
           if (gIperfState.state == (uint8_t)IPERF_UDP_TX_DONE_STATE)
			...
			msec = ((double) (gIperfState.stopTime - gIperfState.startTime)) / (((double)(SYS_TMR_TickPerSecond()))/1000);
...
static void StateMachineTxArpResolve(void)
{
  ...
  if(!TCPIP_ARP_IsResolved(gIperfState.pNetIf, &gIperfState.remoteSide.remoteIPaddress.v4Add, &gIperfState.remoteMACAddr))
  {
      ...
      if( SYS_TMR_TickCountGet() - gIperfState.timer > 5*SYS_TMR_TickPerSecond() )
      {
...
static void GenericTxHeaderPreparation(uint8_t *pData, bool isTheLastTransmit)
{
    ...
    pPktInfo->tv_sec = TCPIP_Helper_htonl(currentTime / SYS_TMR_TickPerSecond());

    /* get the remainder of the ticks using modulus */
    tmp2 = ((gIperfState.stopTime - gIperfState.startTime)%SYS_TMR_TickPerSecond());

    /* normalize  to uSecs */
    tmp2 =  tmp2*1000/SYS_TMR_TickPerSecond(); /* Convert to mSec */
        ...
        case UDP_PROTOCOL:
            ...
            if ( TCPIP_UDP_TxPutIsReady(gIperfState.udpSock, gIperfState.mDatagramSize) < gIperfState.mDatagramSize )
            {
                ...
                {     // beginning wait period
                    gIperfState.txWaitTick = SYS_TMR_TickCountGet() + ((IPERF_TX_WAIT_TMO * SYS_TMR_TickPerSecond()) + 999)/1000;
                    return IPERF_TX_WAIT;
                }
                else if(SYS_TMR_TickCountGet() < gIperfState.txWaitTick)
                { // wait some more
                    return IPERF_TX_WAIT;
                }
...
static void GenericTxEnd(void)
{
    ...
    else
    {
        ...
        if ( gIperfState.pktCount == 0u )
        {
            ...
            (gIperfState.pCmdIO->pCmdApi->print)(cmdIoParam, "    - Target rate = %ld bps, period = %ld ms\r\n",
                    ...
                    (unsigned long)(gIperfState.mPktPeriod*1000/SYS_TMR_TickPerSecond()) );
    ...
    if ( SYS_TMR_TickCountGet() > (gIperfState.lastCheckTime + gIperfState.mInterval - TIMING_ERROR_MARGIN) )
    {
...
static void StateMachineTcpRx(void)
{
    ...
    if ((gIperfState.pktId != (long)0) &&
       (SYS_TMR_TickCountGet() > (gIperfState.lastCheckTime + gIperfState.mInterval)) )
    {
         // Time to report statistics
         ReportBW_Jitter_Loss(INTERVAL_REPORT);
    }
...
static void StateMachineUdpRxDone(void)
{
    ...
    if ( TCPIP_UDP_TxPutIsReady(gIperfState.udpSock, 128 ) > 0u )
    {
      ...
      pServer_hdr->stop_sec =  TCPIP_Helper_htonl( (uint32_t) (gIperfState.stopTime - gIperfState.startTime)/SYS_TMR_TickPerSecond());
...
static int CommandIperfStart(_CMDIO_DEV_NODE* pCmdIO, int argc, char** argv)
{
    ...
    gIperfState.mDuration = ((uint32_t) 10)*((uint32_t) SYS_TMR_TickPerSecond()); // -t: default 10 sec.
    gIperfState.mInterval =  SYS_TMR_TickPerSecond(); 	// -i: default 1 sec.
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        else if ((memcmp(argv[i], "-t", 2) == 0) || (memcmp(argv[i], "--time", 5) == 0) )
        {
            ...
            gIperfState.mDuration = values[0]*SYS_TMR_TickPerSecond();
            gIperfState.mAmount = 0;
        }
        ...
        else if ((memcmp(argv[i], "-i", 2) == 0) || (memcmp(argv[i], "--interval", 5) == 0) )
        {
            ...
            gIperfState.mInterval = values[0]*SYS_TMR_TickPerSecond(); // Convert to msec
        }
#if defined(TCPIP_STACK_USE_UDP)
    ...
    switch (gIperfState.mServerMode)
    {
        case 0:
            ...
            gIperfState.mPktPeriod =  (uint32_t) ( (float) SYS_TMR_TickPerSecond() / pktRate );
