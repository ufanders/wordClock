...
PROTECTED_SINGLE_LIST                 ipv6QueuedPackets = { {0} };
...
static void _TCPIP_IPV6_PacketEnqueue(IPV6_PACKET * pkt, PROTECTED_SINGLE_LIST* pList, int queueLimit, int tmoSeconds);
...
static void _TCPIP_IPV6_Cleanup(const void* memH)
{
    ...
    TCPIP_IPV6_SingleListFree (&ipv6QueuedPackets.list);
    ipv6MemH = 0;
}
...
void TCPIP_IPV6_Deinitialize(const TCPIP_STACK_MODULE_CTRL* const stackCtrl)
{
    ...
    if(ipv6ModuleInitCount)
    {
        ...
        {   // stack shut down
            TCPIP_IPV6_SingleListFree (&mcastQueue.list);
...
unsigned short TCPIP_IPV6_TxIsPutReady (IPV6_PACKET * ptrPacket, unsigned short count)
{
    ...
    payloadSize = (count > IPV6_DEFAULT_ALLOCATION_BLOCK_SIZE)?count:IPV6_DEFAULT_ALLOCATION_BLOCK_SIZE;
    ...
    if(payloadSize < IPV6_DEFAULT_ALLOCATION_BLOCK_SIZE)
    {
        payloadSize = IPV6_DEFAULT_ALLOCATION_BLOCK_SIZE;
    }
...
unsigned short TCPIP_IPV6_ArrayPutHelper (IPV6_PACKET * ptrPacket, const void * dataSource, uint8_t dataType, unsigned short len)
{
    ...
    while(len != 0)
    {
        ...
        { // allocate new segment
            allocSize = (len < IPV6_DEFAULT_ALLOCATION_BLOCK_SIZE) ? IPV6_DEFAULT_ALLOCATION_BLOCK_SIZE : len;
...
static void _TCPIP_IPV6_PacketEnqueue(IPV6_PACKET * pkt, PROTECTED_SINGLE_LIST* pList, int queueLimit, int tmoSeconds)
{
    ...
    ptrPacket = (IPV6_PACKET *)pList->list.head;
    ...
    if (ptrPacket == NULL)
    {
        if (pList->list.nNodes == queueLimit)
        {
            ptrPacket = (IPV6_PACKET *)TCPIP_Helper_ProtectedSingleListHeadRemove (pList);
            if (ptrPacket->ackFnc)
            {
                (*ptrPacket->ackFnc)(ptrPacket, false, ptrPacket->ackParam);
            }
        }
        TCPIP_Helper_ProtectedSingleListTailAdd (pList, (SGL_LIST_NODE *)pkt);
        pkt->flags.queued = true;
        pkt->queuedPacketTimeout = SYS_TMR_TickCountGet() + (SYS_TMR_TickPerSecond() * tmoSeconds);
...
static void _TCPIP_IPV6_QueuedPacketTransmitTask (PROTECTED_SINGLE_LIST* pList)
{
    ...
    uint32_t tick = SYS_TMR_TickCountGet();
    while (pList->list.nNodes != 0)
    {
        queuedPacket = (IPV6_PACKET *)pList->list.head;
        if ((long)(tick - queuedPacket->queuedPacketTimeout) > 0)
        {
            // Remove packet
            queuedPacket->flags.queued = false;
            TCPIP_Helper_ProtectedSingleListHeadRemove (pList);
    ...
    for (i = 0; i < nStackIfs; i++)
    {
        ...
        if(TCPIP_STACK_NetworkIsUp(pNetIf))
        {
            ...
            while (ptrPrefix != NULL)
            {
                ...
                timeElapsed /= SYS_TMR_TickPerSecond();
                correctedCurrentTime = currentTickTime - (timeElapsed % SYS_TMR_TickPerSecond());
            ...
            {
                if ((long)(currentTickTime - ptrRouter->tickTimer) >= SYS_TMR_TickPerSecond())
                {
                    ptrRouter->tickTimer += SYS_TMR_TickPerSecond();
...
static void TCPIP_IPV6_UlaTask (void)
{
    ...
    switch(ulaState)
    {
        ...
        case TCPIP_IPV6_ULA_CONN_START:
            ...
            if(currTick - ulaOperStartTick < (SYS_TMR_TickPerSecond() * IPV6_ULA_NTP_ACCESS_TMO + 999)/1000)
            {   // can try again
                return;
            }
        ...
        case TCPIP_IPV6_ULA_TSTAMP_GET:
            ...
            if(sntpRes < 0 || tStampWindow > (SYS_TMR_TickPerSecond() * IPV6_ULA_NTP_VALID_WINDOW + 999)/1000)
            {   // failed
                if(currTick - ulaOperStartTick < (SYS_TMR_TickPerSecond() * IPV6_ULA_NTP_ACCESS_TMO + 999)/1000)
