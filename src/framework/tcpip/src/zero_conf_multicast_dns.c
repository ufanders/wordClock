                      ...
                      ,DNSDesc_t       *pDNSdesc)
{
   ...
   if ( tag != MDNS_RR_GROUP_QD )
   {
       ...
       default:
          ...
          TCPIP_UDP_ArrayGet(pDNSdesc->mDNS_socket, NULL, i);
                ...
                switch (zgzc_wait_for(&(pCtx->random_delay), &(pCtx->event_time), &(pCtx->time_recorded)))
                {
                    case ZGZC_STARTED_WAITING:
                        ...
                        if (pCtx->state == MDNS_STATE_PROBE)
                        {
                            ...
                            {
                                pCtx->random_delay = (MDNS_PROBE_INTERVAL * (SYS_TMR_TickPerSecond() / 1000));
                            }
                            else
                            {
                                pCtx->random_delay = (SYS_TMR_TickPerSecond());
                            }
                        ...
                        if (pCtx->nClaimCount == 1)
                        {
                            ...
                            pCtx->random_delay = (MDNS_ANNOUNCE_WAIT * (SYS_TMR_TickPerSecond() / 1000));
                        }
                        else
                        {
                            pCtx->random_delay = (MDNS_ANNOUNCE_INTERVAL * (SYS_TMR_TickPerSecond() / 1000));
                        }
