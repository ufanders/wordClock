...
#define ICMPV6_TIMEOUT  (4ul*SYS_TMR_TickPerSecond())
                       ...
                       const void* icmpv6Data)
{
    ...
    if (icmpv6Init == false)
    {
        ...
        TCPIP_Helper_ProtectedSingleListInitialize(&icmpv6RegisteredUsers);
        icmpv6Init = true;
    }
        ...
        case ICMPV6_ERROR_PACKET_TOO_BIG:
            ...
            if (pseudoHeader.DestAddress.v[0] == 0xFF)
            {
                ...
                if (pIpv6Config->multicastMTU > h.header_Error.additionalData)
                {
                    ...
                    pIpv6Config->mtuIncreaseTimer = SYS_TMR_TickCountGet() + (SYS_TMR_TickPerSecond() * IPV6_MTU_INCREASE_TIMEOUT);
                }
            }
            else
            {
