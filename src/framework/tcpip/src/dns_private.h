...
typedef struct
{
    ...
    IP_MULTI_ADDRESS  DNSServers[2];				  // servers associated with the current interface
    int				vDNSServerIx;					// current server used
    uint32_t            dnsTimeSeconds;         // coarse DNS time keeping, seconds
    tcpipAsyncHandle dnsTimerHandle;
    int              dnsInitCount;               // module initialization count
    const void 		 *memH;
    ...
    DNSCLINTBUFFERDATA  udpPutBufferData;  // is used to put UDP data
    DNSCLINTBUFFERDATA  udpGetBufferData; // is used to Get UDP data
    uint8_t*            pDnsHostName; // host name
    uint8_t             recordType;
#ifdef TCPIP_STACK_USE_IPV6
