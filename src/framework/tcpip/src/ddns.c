              ...
              const DDNS_MODULE_CONFIG* ddnsData)
{
    ...
    if (!IfUp)
    {
        ...
        dwUpdateAt = SYS_TMR_TickCountGet() + 15*SYS_TMR_TickPerSecond();
        bForceUpdate = true;
...
void TCPIP_DDNS_Task(void)
{
    ...
    switch(smDDNS)
    {
        case SM_IDLE:
            ...
            if(SYS_TMR_TickCountGet() > dwUpdateAt)
                break;
        ...
        case SM_CHECKIP_SKT_OBTAINED:
            ...
            if(!TCPIP_TCP_IsConnected(MySocket))
            {
                ...
                if(SYS_TMR_TickCountGet()-DDnsTimer > 6*SYS_TMR_TickPerSecond())
                {
        ...
        case SM_CHECKIP_FIND_DELIMITER:
            ...
            if(!TCPIP_TCP_IsConnected(MySocket) || SYS_TMR_TickCountGet() - DDnsTimer > 6*SYS_TMR_TickPerSecond())
        ...
        case SM_IP_UPDATE_REQ_A:
            ...
            if(!TCPIP_TCP_IsConnected(MySocket) || (SYS_TMR_TickCountGet() - DDnsTimer > 10*SYS_TMR_TickPerSecond()))
            {
                lastStatus = DDNS_STATUS_UPDATE_ERROR;
        ...
        case SM_IPUPDATE_FIND_RESPONSE:
            ...
            if(SYS_TMR_TickCountGet() - DDnsTimer > 10*SYS_TMR_TickPerSecond())
            {
                lastStatus = DDNS_STATUS_UPDATE_ERROR;
                smDDNS = SM_IPUDATE_DISCONNECT;
                break;
        ...
        case SM_DONE:
            dwUpdateAt = SYS_TMR_TickCountGet() + 10*60*SYS_TMR_TickPerSecond();   // 10 minutes
            smDDNS = SM_IDLE;
            break;
        ...
        case SM_DNS_ERROR:
        case SM_SKT_ERROR:
            dwUpdateAt = SYS_TMR_TickCountGet() + 30*SYS_TMR_TickPerSecond();      // 30 seconds
            smDDNS = SM_IDLE;
            break;

        case SM_SYSTEM_ERROR:
            dwUpdateAt = SYS_TMR_TickCountGet() + 30*60*SYS_TMR_TickPerSecond();       // 30 minutes
            smDDNS = SM_IDLE;
            break;
    }
}
