    ...
    switch(TransportState)
    {
        ...
        case TRANSPORT_SECURING_SOCKET:
            if(!TCPIP_TCP_IsConnected(MySocket))
            {
                ...
                // to the SMTP server
                if((SYS_TMR_TickCountGet()-SMTPTimer) > (SMTP_SERVER_REPLY_TIMEOUT * SYS_TMR_TickPerSecond()))
                {
        ...
        case TRANSPORT_SOCKET_OBTAINED:
            if(!TCPIP_TCP_IsConnected(MySocket))
            {
                ...
                // Also time out if we can't establish the connection
                // to the SMTP server
                if(SMTPFlags.bits.ConnectedOnce || ((SYS_TMR_TickCountGet()-SMTPTimer) > (SMTP_SERVER_REPLY_TIMEOUT * SYS_TMR_TickPerSecond())))
                {
                    ResponseCode = SMTP_CONNECT_ERROR;
                    TransportState = TRANSPORT_CLOSE;
                }
